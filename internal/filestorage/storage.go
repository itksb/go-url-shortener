...
func (s *storage) SaveURL(ctx context.Context, url string) (string, error) {
	...
	if err := s.persist(id, url); err != nil {
...
func (s *storage) findByID(id int64) (string, bool) {
	...
	for reader.Scan() && len(foundValue) == 0 {
		line = reader.Text()
		curID, url, ok := extractValuesFromTheLine(line)
		if ok && curID == id {
			foundValue = url
		}
	}
...
func extractValuesFromTheLine(line string) (int64, string, bool) {
	res := strings.SplitN(line, "::", 2)
	if len(res) == 2 {
		curID, err := strconv.ParseInt(res[0], 10, 64)
		if err == nil {
			return curID, res[1], true
		}
	}
	return 0, "", false
}

func (s *storage) persist(id int64, value string) error {
	_, err := s.fileWrite.WriteString(fmt.Sprintf("%d::%s\n", id, value))
	return err
}
...
func getLastIDOrDefault(file *os.File) (int64, error) {
	...
	id, _, ok := extractValuesFromTheLine(line)
	if ok {
		return id, err
	}
